...
def getarbitraryCouplings( currentPermAsTable, newPerm, W,N):
    ...
    dim= len(newPerm) #number of cycles
    ...
    #Chose the i-th and the j-th cycle to compute the weight $\Tilde{ W}_{i,j}$ between the binary variables \alpha_i and \alpha_j   
    for count, cycle in enumerate(newPerm):
        for count2, cycle2 in enumerate( newPerm):
        
            placeList1=[]#at what places does the cycle occur?(support) 
            placeList2= []
...
#s. formula (6) in the paper variablePlaces will be (c_i-I)p_0
...
def RunAnnealSim(W,b):
    ...
    num_reads=100
    response = solver.sample_ising(bias, J ,num_reads=num_reads )
    Result=[]
...
def optimize(W, initial, CycleSuggestions,iterationNumber ,probl ):
    """
    ...
    if initial==None:
        ...
        initialPermutation= (initial[:,1]-1).tolist()
    ...
    for i in range(len(CycleLists)):
             ...
             [Result,info]=  RunAnnealSim(We,ce)
             change= Result
             Save.append(We)
             Save.append(ce)
             Save.append(Result)
             Save.append(info)
             Save.append(currentPermutationTable)
    ...
    pickle.dump( Save , open( "probl"+str(probl)+"save"+str(iterationNumber)+".p", "wb" ) )
